...
//         const wishlist = await Wishlist.findOne({ userId: userId }).populate({path:'products.productId',match:{isBlocked:true}});
...
exports.getWishListPage = async (req, res) => {
    try {
        ...
        const wishlist = await Wishlist.findOne({ userId: userId })
            .populate('products.productId'); // Fetch all products

        if (!wishlist) {
            console.log('Wishlist not found');
            return res.render('wishList', { wishlistItems: [], cartCount, wishlistCount: 0 });
        }

        // Filter out blocked products and remove them from the wishlist
        const unblockedProducts = wishlist.products.filter(item => !item.productId.isBlocked);

        // If there are blocked products, remove them from the wishlist in the database
        if (unblockedProducts.length !== wishlist.products.length) {
            wishlist.products = unblockedProducts;
            await wishlist.save();
        }
        ...
        // Pass the unblocked wishlist items to the EJS template
        res.render('wishList', { wishlistItems: wishlist.products, cartCount, wishlistCount });
    } catch (error) {
        console.error('Error retrieving wishlist:', error.stack);
        res.status(500).render('error', { message: 'Server error while retrieving wishlist.' });
